...
def init_pool():
    global _pool
    if not DATABASE_URL:
        print("[WARNING] DATABASE_URL not set — skipping pool init")
        return
    ...
    print("[pool] Connection pool ready (min=2, max=10)")
...
def init_all_db():
    if not DATABASE_URL:
        print("[WARNING] DATABASE_URL not set — skipping init_all_db()")
        return
...
def _init_core():
    try:
        ...
        print(f"[ERROR] _init_core failed: {e}")
        raise
...
def _init_salary():
    ...
    try:
        with get_db() as conn:
            if conn.execute("SELECT COUNT(*) as c FROM salary_items").fetchone()['c'] == 0:
...
def _seed_tw_holidays():
    holidays = [
        ('2025-01-01', '元旦'), ('2025-01-27', '農曆除夕'),
        ('2025-01-28', '春節'), ('2025-01-29', '春節'), ('2025-01-30', '春節'),
        ('2025-01-31', '春節補假'), ('2025-02-28', '和平紀念日'),
        ('2025-04-03', '兒童節補假'), ('2025-04-04', '兒童節/清明節'),
        ('2025-05-01', '勞動節'), ('2025-05-30', '端午節補假'),
        ('2025-06-02', '端午節'), ('2025-10-06', '中秋節補假'),
        ('2025-10-07', '中秋節'), ('2025-10-10', '國慶日'),
        ('2026-01-01', '元旦'), ('2026-01-28', '農曆除夕'),
        ('2026-01-29', '春節'), ('2026-01-30', '春節'), ('2026-01-31', '春節'),
        ('2026-02-02', '春節補假'), ('2026-02-28', '和平紀念日'),
        ('2026-03-02', '和平紀念日補假'), ('2026-04-03', '兒童節'),
        ('2026-04-04', '清明節'), ('2026-04-05', '清明節補假'),
        ('2026-05-01', '勞動節'), ('2026-06-19', '端午節'),
        ('2026-09-25', '中秋節'), ('2026-10-09', '國慶日補假'),
        ('2026-10-10', '國慶日'),
    ]
    try:
        with get_db() as conn:
...
def _init_finance():
    ...
    try:
        with get_db() as conn:
            if conn.execute("SELECT COUNT(*) as c FROM finance_categories").fetchone()['c'] == 0:
