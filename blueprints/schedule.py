...
from helpers import (
    sched_req_row, shift_type_row, shift_assign_row,
    get_schedule_config, get_off_counts, _notify_staff_line,
)
...
def api_sched_submit():
    ...
    if not month:
        return jsonify({'error': '請選擇月份'}), 400
...
def api_shift_import():
    ...
    with get_db() as conn:
        ...
        if not force:
            ...
            for lr in leave_rows:
                ...
                if sid_lr not in leave_lookup:
                    leave_lookup[sid_lr] = set()
                leave_lookup[sid_lr].update(dates_val or [])
    ...
    if errors or skipped:
        result['message'] = (f'匯入完成：{created} 筆成功，'
                             f'{len(skipped)} 筆排休衝突跳過，{len(errors)} 筆錯誤')
    else:
...
def api_shift_export():
    ...
    for d in range(1, days_in_month + 1):
        ...
        c.fill      = (PatternFill('solid', fgColor='8B2020')
                       if (is_wkend or is_hol)
                       else PatternFill('solid', fgColor='1A3060'))
        c.alignment = center
    ...
    for row_idx, staff in enumerate(staff_list, start=2):
        ...
        for col, val in enumerate(
            [staff['name'], staff['employee_code'] or '', staff['role'] or ''], start=1
        ):
        ...
        for d in range(1, days_in_month + 1):
            ...
            c.border    = thin_border
            if not val:
                c.fill = (wkend_fill if wd >= 5
                          else hol_fill if ds in holidays
