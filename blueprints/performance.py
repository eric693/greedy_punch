...
def api_perf_template_create():
    ...
    with get_db() as conn:
        ...
        """, (name, b.get('description', ''), b.get('period', 'quarterly'),
              _json.dumps(b.get('items', [])))).fetchone()
...
def api_perf_review_create():
    ...
    from helpers import _notify_staff_line
...
def api_perf_adjust_salary(rid):
    ...
    with get_db() as conn:
        ...
        conn.execute("UPDATE punch_staff SET base_salary=%s WHERE id=%s", (new_salary, staff['id']))
        conn.execute(
            "UPDATE performance_reviews SET salary_adjusted=TRUE, salary_delta=%s WHERE id=%s",
            (delta, rid)
        )
