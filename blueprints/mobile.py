...
def _get_jwt_secret():
    ...
    import os
    return os.environ.get('MOBILE_JWT_SECRET', SECRET_KEY)


def _make_jwt(payload: dict) -> str:
    import os
    from datetime import timezone, timedelta
    payload['exp'] = _dt.now(timezone.utc) + timedelta(hours=_JWT_EXPIRE_HOURS)
...
def mobile_jwt_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            ...
            if 'Expired' in str(type(e).__name__) or 'expired' in str(e).lower():
...
def mobile_me():
    ...
    if u['role'] == 'employee':
        ...
        if not staff: return jsonify({'error': '帳號不存在'}), 404
        d = dict(staff)
        ...
        if not admin: return jsonify({'error': '帳號不存在'}), 404
        d = dict(admin)
...
def mobile_punch_status():
    u = g.mobile_user
    if u['role'] != 'employee': return jsonify({'error': '僅員工可查詢'}), 403
    staff_id = int(u['sub'])
...
def mobile_leave_apply():
    u = g.mobile_user
    if u['role'] != 'employee': return jsonify({'error': '僅員工可申請'}), 403
    staff_id      = int(u['sub'])
    b             = request.get_json(force=True) or {}
...
def mobile_overtime():
    ...
    hours    = b.get('hours')
    reason   = b.get('reason', '')
    if not ot_date or not hours: return jsonify({'error': '缺少必填欄位'}), 400
    try:
    ...
    with get_db() as conn:
        conn.execute(
            """INSERT INTO overtime_requests (staff_id, request_date, ot_hours, reason, status)
...
def mobile_admin_leave_action(lid):
    b      = request.get_json(force=True) or {}
    action = b.get('action')
    if action not in ('approve', 'reject'): return jsonify({'error': '無效操作'}), 400
