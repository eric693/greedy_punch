...
def _get_rp_and_origin():
    ...
    if not rp_id or not origin:
        ...
        host   = request.host
        if not rp_id:   rp_id   = host.split(':')[0]
        if not origin:  origin  = f"{scheme}://{host}"
    return rp_id, origin
...
def webauthn_register_begin():
    ...
    rp_id, origin = _get_rp_and_origin()
...
def webauthn_register_complete():
    ...
    rp_id         = session.get('webauthn_reg_rp_id', '')
    origin        = session.get('webauthn_reg_origin', '')
    ...
    try:
        ...
        assert rp_id_hash == _hs2.sha256(rp_id.encode()).digest(), 'rpIdHash mismatch'
        flags = auth_data[32]
...
def webauthn_auth_complete():
    ...
    rp_id         = session.get('webauthn_auth_rp_id', '')
    origin        = session.get('webauthn_auth_origin', '')
    ...
    try:
        ...
        assert auth_data[:32] == _hs3.sha256(rp_id.encode()).digest(), 'rpIdHash mismatch'
        flags = auth_data[32]
        assert flags & 0x01, 'User Presence not set'
        assert flags & 0x04, 'User Verification not set'
        ...
        _verify_cose_signature(bytes(cred['public_key']), auth_data + client_data_hash, signature)
        ...
        if user_key.startswith('admin_'):
            ...
            if not admin: return jsonify({'error': '帳號不存在或已停用'}), 401
            import json as _jx
            perms = admin['permissions']
            if isinstance(perms, str):
                try: perms = _jx.loads(perms)
                except: perms = []
            ...
            if not staff: return jsonify({'error': '帳號不存在或已停用'}), 401
