...
from helpers import _gps_distance

bp = Blueprint('line_bot', __name__)

CUSTOM_RICHMENU_IMAGE_PATH = '/tmp/custom_richmenu.png'
_pending_line_punches = {}   # {line_user_id: (punch_type, date)}
_reply_token_map      = {}   # {line_user_id: reply_token}
    ...
    if msg_type == 'location':
        ...
        if punch_type:
            ...
            if gps_required and locs:
                from linebot import LineBotApi
                from linebot.models import TextSendMessage, QuickReply, QuickReplyButton, LocationAction
                ...
                if cfg_lp and cfg_lp.get('enabled') and cfg_lp.get('channel_access_token'):
                    ...
                    try:
                        ...
                        token = _reply_token_map.pop(user_id, None)
                        if token:
                            api.reply_message(token, qmsg)
                        else:
                            api.push_message(user_id, qmsg)
...
def api_richmenu_create():
    ...
    btn_texts  = [
        (btn_texts[i].strip() if i < len(btn_texts) and btn_texts[i].strip() else defaults[i])
        for i in range(4)
    ]
...
def _line_submit_leave(staff, user_id, text):
    ...
    if len(parts) == 2:
        ...
        for i in range(7):
            ...
            label = ('今天 ' if i == 0 else '明天 ' if i == 1 else '') + \
                    f'{d.strftime("%m/%d")}({WDAY_LV[d.weekday()]})'
    ...
    if (len(parts) == 5 and parts[3] == '指定時段'
            and _re_lv.match(r'^\d{2}:\d{2}$', parts[4])):
        leave_type_name = parts[1]
        date_str        = parts[2]
        start_t         = parts[4]
        sh, sm   = int(start_t[:2]), int(start_t[3:])
        base     = _tdlv(hours=sh, minutes=sm)
        durations = [0.5, 1, 1.5, 2, 2.5, 3, 4, 5, 6, 7, 8]
        items = []
        for d in durations:
            end_dt  = _tdlv(seconds=(base + _tdlv(hours=d)).total_seconds())
            tot_min = int(end_dt.total_seconds() // 60)
            eh, em  = tot_min // 60, tot_min % 60
    ...
    if period_token == '上午':
        ...
        start_half = True;  end_half = False
    ...
    try:
        ...
        _send_line_punch(user_id,
            f'日期格式錯誤，請使用 YYYY-MM-DD，例：{_dlv.today().isoformat()}')
        return
    ...
    with get_db() as conn:
        ...
        s    = _dlv.fromisoformat(date_str1)
        e    = _dlv.fromisoformat(date_str2)
        ...
        """, (staff['id'], lt['id'], date_str1, date_str2, days,
              start_half, end_half, '（LINE 請假）')).fetchone()

    period_label = (
        '（上午半天）' if (not start_half and end_half and date_str1 == date_str2) else
...
def _line_query_performance(staff, user_id):
    ...
    grade_labels = _get_grade_labels_from_db()
    ...
    _send_line_punch(user_id,
        ...
        f'評級：{row["grade"]} {grade_labels.get(row["grade"], "")}'
        f'{adj}\n'
        + (f'備注：{row["comments"][:60]}\n' if row['comments'] else '')
        + f'考核日：{reviewed}')


def _get_grade_labels_from_db():
    _DEFAULT = [
        {'grade': 'S', 'label': '優秀', 'min_pct': 90},
        {'grade': 'A', 'label': '良好', 'min_pct': 80},
        {'grade': 'B', 'label': '普通', 'min_pct': 60},
        {'grade': 'C', 'label': '待改進', 'min_pct': 0},
    ]
    try:
        with get_db() as conn:
            row = conn.execute(
        ...
        if row:
            ...
            if isinstance(cfg, list) and cfg:
                return {c['grade']: c['label'] for c in cfg}
    except Exception:
        pass
    return {c['grade']: c['label'] for c in _DEFAULT}
...
def _line_query_monthly_records(staff, user_id, text):
    ...
    for ds in sorted(days_map.keys()):
        recs      = days_map[ds]
    ...
    header = (f'📋 {staff["name"]} {month} 出勤\n'
              f'出勤 {len(days_map)} 天｜工時 {total_str}{anomaly_str}\n'
              + '─' * 20)
...
def _line_submit_overtime(staff, user_id, text):
    ...
    try:
        ...
        _send_line_punch(user_id,
            f'日期格式錯誤，請使用 YYYY-MM-DD，例：{_dot.today().isoformat()}')
        return
    ...
    if len(parts) == 3:
        ...
        items = [
            {'label': f'至 {t}（+{d}h）', 'text': f'申請加班 {date_str} {start_str} {t}'}
    ...
    with get_db() as conn:
        row = conn.execute("""
            ...
            RETURNING id
        """, (staff['id'], date_str, start_str, end_str,
              round(hours, 2), '（LINE 加班申請）')).fetchone()
...
def _line_show_leave_types(staff, user_id):
    ...
    if not rows:
        _send_line_punch(user_id, '目前無可用假別。')
        return
    lines = ['🗂️ 可用假別清單\n']
    for r in rows:
